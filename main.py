...
USER_AGENT = "IPTV-Argentina-Espana/8.0"
SOURCE_TIMEOUT = 30
PROBE_TIMEOUT = 8
WORKERS = 24
...
def norm(s):
    s=unicodedata.normalize("NFKD",str(s or "")); return "".join(c for c in s if not unicodedata.combining(c)).lower().strip()
...
def set_group_if_missing(line,g):
    # No reescribir metadata existente: algunos reproductores y listas dependen del EXTINF original.
    if attr(line,"group-title").strip(): return line
    p=line.rfind(","); return line[:p]+f' group-title="{g}"'+line[p:] if p>=0 else line

def stream_url(u): return u.strip().lower().startswith(("http://","https://","rtmp://","rtmps://","rtsp://","udp://","srt://","acestream://"))
...
def probe_hls(request_url,r,h):
    ...
    except Exception: return False,"cannot read playlist"
    if not text.lstrip().startswith("#EXTM3U"): return False,"not an M3U response"
    media=any(x.startswith("#EXTINF:") for x in text.splitlines()); variants=any("#EXT-X-STREAM-INF" in x for x in text.splitlines())
...
def probe(item):
    ext,url,directives,priority,source,source_order,entry_order=item; k=kind(url)
            ...
            try:
                with requests.get(request_url,stream=True,timeout=(PROBE_TIMEOUT,PROBE_TIMEOUT),headers=h,allow_redirects=True,verify=verify) as r:
                    ...
                    if code>=400:
                        ...
                        continue
                    if k==0 and (request_url.lower().endswith((".m3u8",".m3u")) or "mpegurl" in (r.headers.get("content-type") or "").lower()): ok,msg=probe_hls(request_url,r,h)
                    else:
                        ctype=(r.headers.get("content-type") or "").lower(); chunk=next(r.iter_content(8192),b""); ok=bool(chunk) and "text/html" not in ctype; msg="ok" if ok else "empty/html response"
                    last=msg
                    if ok: successes=1; break
...
def main():
    ...
    candidates=[]; good_sources=0; entry_order=0
    for source_order,src in enumerate(cfg.get("sources",[])):
        ...
        for ext,u,d in parse(r.text.splitlines()):
            ...
            candidates.append((ext,u,d,priority,source,source_order,entry_order)); entry_order+=1
    ...
    else: results=[(x,kind(x[1])==0,999.0,"not probed",0,kind(x[1])) for x in candidates]

    # CRÍTICO: no deduplicar por URL sola. La misma URL puede representar canales distintos
    # o necesitar metadata distinta. Solo eliminar una entrada completamente idéntica.
    seen=set(); unique=[]
    for item,ok,lat,status,successes,k in results:
        ext,u,d,p,src,so,eo=item; output_ext=set_group_if_missing(ext,section(ext)); key=(output_ext,u.strip(),tuple(d))
        if key in seen: continue
        seen.add(key); unique.append((output_ext,u,d,p,src,ok,lat,successes,k,so,eo))
    ...
    sections={}
    for x in unique: sections.setdefault(section(x[0]),[]).append(x)
    ordered=[s for s in preferred if s in sections]+sorted(s for s in sections if s not in preferred)
    out=["#EXTM3U"]
    for sec in ordered:
        items=sections[sec]
        # Mantener juntas las variantes de un mismo canal y priorizar las directas verificadas.
        # source/entry order rompe empates y conserva el orden original de cada fuente.
        items.sort(key=lambda x:(norm(name(x[0])),x[8],not x[5],x[6],x[3],x[9],x[10]))
        for ext,u,d,*_ in items: out.extend([ext,*d,u])
    Path("playlist.m3u").write_text("\n".join(out)+"\n",encoding="utf-8")
    logging.info("Playlist: %d entradas exactas; variantes y metadata originales preservadas",len(unique))
    return 0
